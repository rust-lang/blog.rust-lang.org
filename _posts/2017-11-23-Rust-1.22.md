...
If you have a previous version of Rust installed, getting Rust 1.22 is as easy as:
...
The headline feature for this release is one many have been waiting for
for a long time: you can now [use `?` with `Option<T>`]! About a year
ago, in [Rust 1.13], we introduced the `?` operator for working with
`Result<T, E>`. Ever since then, there's been discussion about how far
`?` should go: should it stay only bound to results? Should it be
user-extensible? Should it be usable with `Option<T>`?
...
example. In the future, this will be possible, and already is in nightly
...
On its own, this change doesn't bring much, but as we improve our
...
Previously, you'd have needed to write `x += *y` in order to de-refernece, so
...
sometimes, additions to Rust cause code that previously wouldn't have compiled
into code that does. Consider the first announcement with `?`, for example:
that code would fail to compile on Rust 1.21, but compile successfully on
Rust 1.22, causing your test suite to start failing.
