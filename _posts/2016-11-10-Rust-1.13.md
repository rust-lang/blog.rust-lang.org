...
The unstable `Reflect` trait is deprecated, which reflects a change in the
Rust language's approach to parametricity.

1.13 contains a [serious bug in code generation][arm] for ARM targets using hard
floats (which is most ARM targets). Because 1.13 contains a security update,
users that must target ARM are encouraged to use the 1.14 betas, which will soon
get a fix for ARM.
