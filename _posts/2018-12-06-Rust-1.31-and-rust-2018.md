...
Cargo will now download packages in parallel using HTTP/2. Additionally, now
that `extern crate` is not usually required, it would be jarring to do
`extern crate foo as bar;` to rename a crate. As such, you can do so in your
