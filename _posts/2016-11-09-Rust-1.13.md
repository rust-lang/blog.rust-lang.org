...
It's been a really busy season in Rust. We just powered through three Rust
conferences in short succession, and all the while we've been hacking
away. There's a whole bunch of amazing stuff coming down the pipeline, but in
today's release we're highlighting three new language features, including the
long-awaited `?` operator, as well as a surge in progress on compiler
performance.

#### The `?` operator

Rust has gained a new operator, `?`, that makes error-handling more pleasant by
reducing the visual noise around to handle errors in Rust. It does this by
...
ideomatic Rust. It is used so often that we think it's worth having a sweet
...
prefixs the expression with an additional call to `try!`. This brings undo
...
#### Type macros and statement attributes
...
now generates the following minimal LLVM IR:
...
Your eyes don't deceive you. This crate, though it contains many type and
function definitions, contains no machine code at all, because all its functions
are marked `#[inline]`.
