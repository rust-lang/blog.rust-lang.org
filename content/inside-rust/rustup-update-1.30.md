+++
path = "inside-rust/2026/06/29/rustup-update-1.30"
...
seem that old behavior would be clearly unnecessary in certain places, such as
in the following example with rustup 1.27.1, where rustup must "install the
active toolchain first in order to remove a target"while it should have simply
bailed out:
...
invocations (with some exceptions, see the link for more info); it will be in
effect starting from rustup 1.30.

If you have any concerns regarding the above change, please let us know in the
[issue thread][Refining the auto installation behavior]. In the meantime,
_extra deprecation warnings are planned for the upcoming releases of 1.29_,
which should provide everyone with more time to prepare.
