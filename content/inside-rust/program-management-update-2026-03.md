...
After his onboarding, Nurzhan is able (and comfortable) to attend the team meetings alone. That let us split the load evenly while keeping the same PM coverage. We can cover for absences, but overall our personal meeting strain has dropped by half.

This is great! As wonderful as it is to be in the room where things are being decided, help people out, and record the discussions for posterity, it is a draining work — especially on days where there are 6–7 hours of back-to-back meetings.

We're still experimenting with the schedule, but we've made a conscious choice to not specialize. We both participate in each team's discussion, get to know people and have them get to know us. That makes it easier to share our knowledge and context, as well as being able to step in when the other one can't make it.
...
On the topic of meeting, two changes happened. First, [Josh Triplett proposed to temporarily pause the Style meetings](https://rust-lang.zulipchat.com/#narrow/channel/346005-t-style/topic/Meeting.202026-03-31/near/582686622). For the last few months (and sporadically for much longer than that), only two Style members were participating.

The team needs more members (as does rustfmt) and ideally a rustfmt liaison on the meeting. We are looking for options, but for the time being, urgent issues can be handled asynchronously.

The other bit of news is that the two Libs-API meetings have merged into one. We used to have one session for items (issues, PRs, etc.) nominated to the team, and a separate one to go through the open [API Change Proposals (ACP)][acp]. These both happened on Tuesdays with an hour in-between.
...
[And now they can!](https://github.com/rust-lang/calendar/pull/114) The two meetings merged which lets everyone to keep going and finish an hour earlier as well. Or more if the discussion is particularly draining.
...
The [FLS](https://rust-lang.github.io/fls/) is a specification of Rust for the use in safety critical environments such as cars, originally developed by [Ferrous Systems](https://ferrous-systems.com/). It was donated to Rust and it is now maintained by the [FLS team](https://github.com/rust-lang/fls-team).
...
This functionality will be implemented behind unstable feature flags, but if it proves out, we can use it to build a more general mirroring support.
...
In December 2025, we [mentioned the maintainer fund][dec-rfmf] and forming the [RFMF Design Committee][rfmf-committee]. They interviewed several open source organizations that have similar programs in place (Python Software Foundation, Scala Center, Django, Zig Software Foundation) and [proposed an RFC][rfmf-rfc].
...
This is complementary to the proposed [Grants program][grants] which provides financial support for twelve months for people already doing valuable work for the Project. They're not being hired/contracted, but we want to recognize their great work and make it easier for them to continue.
...
The Rust project is participating in [Outreachy][outreachy]! It is a paid open source internship program for people under-represented in tech.
...
David Hewitt asked about the Drop with context feature, which would require that an associated context (e.g. pointer to the Python interpreter state) is implicitly passed along to the `drop` function.

This is something the Project has been thinking about (see [Tyler Mandry's 2021 post](https://tmandry.gitlab.io/blog/posts/2021-12-21-context-capabilities/), the recent [What If Traits Carried Values post by Nadrieril](https://nadrieril.github.io/blog/2026/03/22/what-if-traits-carried-values.html), and the related [2026 Dictionary-passing style goal proposal](https://rust-lang.github.io/rust-project-goals/2026/dictionary-passing-style-experiment.html)). David, Josh Triplett, and Jack Huey plan to keep discussing this on Zulip as well as the 2026 All Hands.
...
Emma Smith wanted to confirm whether LLVM and GCC-based sanitizers can't be mixed — and they can't. When using Rust, the C portions of the Python interpreter will have to be built using Clang, and those will need to use its own sanitizers. And we need to make sure that these cover the same ground.

The team also asked about compatibility between LLVM releases given that Rust uses its own LLVM fork. The answer is, while the major versions of LLVM are incompatible, the Rust's fork and the corresponding upstream version should be compatible. Rust carries backported upstream bugfixes and supports building with stock LLVM. 
