...
across several repositories that depend on one another. This post covers our experience and some problems that we encountered with this cross-repository code sharing and describes how we leverage an awesome git extension tool called [Josh][Josh] (Just One Single History), which helps us overcome these challenges.
...
The price that has to be paid to enable this workflow is that we need to periodically perform a bidirectional sync between the parent and each subproject that is embedded as a subtree. This synchronization consists of two operations:

- A `pull` operation backports any changes made to the subproject in the parent repo back into the repository of the subproject.
...
This workflow conceptually worked pretty well for us, but we had a severe problem with the implementation of git subtree; it is *painfully* slow. On repositories like Miri, it could take tens of minutes to finish a sync operation (TODO(Ralf): how slow was it?), and that was not a good experience.

TODO(Ralf): some other disadvantage? different commit SHAs?

Luckily, we found an alternative that is much faster.

## Josh comes to the rescue

[Josh][Josh] is a git extension tool written in Rust that implements sophisticated and performant filtering operations on git repositories. It provides a set of reversible git [algebraic filters][josh-filters] that allow manipulating the history of git repos, which enables several cool use-cases. For example, it offers a git proxy that can transparently split a repository into a set of subrepositories on demand, compose several repositories into a virtualized monorepo or linearize the history of a repository that uses merge commits. Though in our case, we mostly simply use it as a very fast "git subtree on steroids". In fact, our Josh workflow is very similar to the way we use git subtrees, with the biggest difference being that the sync operations are an order of magnitude faster.
