...
There is an important milestone of sorts for Rust for Linux: the upcoming Debian 14 stable release (codename: Forky). It is the point at which the project will eventually be able to raise the minimum supported Rust version and take advantage of features shipped in the meantime. By the time Debian stable is released, other major Linux distributions are usually already on that version or newer, or they will be upgrading very soon. The project's MSRV is then bumped to the version in Debian stable about a year after its release, to give contributors time to upgrade their systems and avoid depending on rustup. Debian releases happen roughly every two years, so we can roughly expect Forky to be released around Summer 2027 and Rust for Linux to be able to use the new features around Summer 2028.
