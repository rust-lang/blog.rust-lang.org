...
In general, something that provides new language capabilities — especially when tied in with new syntax — is something that Rust for Linux wants to use soon so they don't have to rewrite a lot of code later on. If the new syntax can be hidden behind a macro, that is acceptable. Anything behind a feature flag is also acceptable so long as there's a reasonable expectation it will be stabilized roughly in that form (e.g., no massive syntax changes) or won't be removed. For example, immovable types could supersede `Pin`; leading to massive syntactic changes. A special case is the [in-place initialization](https://rust-lang.github.io/rust-project-goals/2026/in-place-init.html) language feature; the team has a working solution for it with a potential path to integrate with the language feature, so it's not as blocking as a [field projections] feature, which doesn't even exist as an unstable feature.
