...
* `array[i]` will check to make sure that `array` as at least `i` elements
...
## Stabilizing function-like procedrual macros in expressions, patterns, and statements

In [Rust 1.30.0](https://blog.rust-lang.org/2018/10/25/Rust-1.30.0.html), we stabilized
"function-like procedrual macros in item position." For example, [the
...
At the same time, Rocket was only able to achieve its power and ergonomics by
